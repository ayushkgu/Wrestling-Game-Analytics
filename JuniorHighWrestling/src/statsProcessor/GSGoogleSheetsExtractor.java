...
public class GSGoogleSheetsExtractor {
	 ...
	 */
	private static Credential getCredGentials(final NetHttpTransport HTTP_TRANSPORT) throws IOException {
		// Load client secrets.
	...
	public void setSheetId(String sid) {
		sheetId = sid;
	}
	...
	private Bout extractDualMatch(String weight, String wrestlerH, String wrestlerV, String resultCell,
			String boutScore, String boutTime, String matchName) throws ExcelExtractorException {
		...
		if (resultCell.equals(GS_DOUBLE_FORFEIT)) {
			...
			throw new ExcelExtractorException("Unknown match result type encountered... <" + resultCell + ">",
					rowNumAt);
		}
		...
		if (homeTeam.equals(team)) {
			...
			throw new ExcelExtractorException("ERROR " + team + " not equal to " + homeTeam + " or " + visitorTeam,
					rowNumAt);
		}
	...
	private GSDualMeet initializeDual(String eventString) throws ExcelExtractorException {
		...
		if (tokens.length != 2) {
			throw new ExcelExtractorException(
					"error in initializeDualString with " + eventString + " length is " + tokens.length, rowNumAt);
		}
	...
	private void processDualHeaderRows(List<List<Object>> resultSheet) throws Exception {
		...
		if (lastCellNum != 7) {
			throw new ExcelExtractorException("Unexpected Header Count of " + lastCellNum + " at " + rowNumAt,
					rowNumAt);
		}
	...
	private int processDualMatches(GSDualMeet d, List<List<Object>> resultSheet, String eventString)
			throws ExcelExtractorException {
		int rowCheck = 0;
				...
				if (rowAt.size() >= 5) {
					...
					if (nameCell.equals(ROSTER_NAME_TOKEN)) {
						...
						if (t.wrestlerExists(nameCell)) {
							throw new ExcelExtractorException("ROSTER: wrestler " + nameCell + " exists already.",
									rowNumAt);
						} else {
	...
	public GSTeam extractResultsForDual(GSTeam theTeam) throws Exception {
		/*
		 * This is the processing of the sheet that has dual and tourney results for a
		 * team on it.
		 */
		rowNumAt = 0;
		...
		/*
		 * This while loop will process chunks of the results. This is why the rowAt is
		 * not incremented
				...
				if (inAnEventFlag == false) {
					...
					if (!firstColOnlyContent(rowAt)) {
						if (garbageRecord(rowAt)) {
							...
							throw new ExcelExtractorException(
									"Something weird PhysicalNumberOfCells is <" + rowAt.size() + "> expecting 0",
									rowNumAt);
						}
					} else {
